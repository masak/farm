...

{
    my $game = Game.new(p => {}, t => {},
                        fd => { <rabbit> }, wd => { <sheep> });
    $game.play_round();
    is_deeply non_rolls($game.e), [], "rolling rabbit/sheep gives you nothing";
}
...

{
    my $game = Game.new(p => {player_1 => { horse => 1, small_dog => 1 }},
                        t => {}, fd => { <rabbit> }, wd => { <wolf> });
    $game.play_round();
    is_deeply non_rolls($game.e), [], "wolf doesn't eat horses and small dogs";
}
